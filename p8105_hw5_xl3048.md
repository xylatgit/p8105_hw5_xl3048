...
output_result %>%
  ...
  geom_point(size=5,alpha=0.7,color = "tomato2")+
...
output_result %>%
  ...
  ggplot(aes(x = true_mu, y = avg_estimate_mu, fill = true_mu))+
  geom_point(size=5,alpha=0.7,color = "blue")+
...
<img src="p8105_hw5_xl3048_files/figure-gfm/unnamed-chunk-8-2.png" width="90%" />

The sample average of μ̂ across tests for which the null is rejected
approximately equal to the true value of μ from the plot denoted by
blue.
